 ...
 */
    ...
    (function () {
        ...
        async function handleCompareTrends() {
            ...
            try {
                ...
                data.trends.forEach(t => {
                    ...
                    let diffIcon = t.change > 0 ? svgIcon("arrow-up") : (t.change < 0 ? svgIcon("arrow-down") : "");
                    let diffPrefix = t.change > 0 ? "+" : "";

                    html += `<tr>
                    <td><strong>${escapeHtml(t.biomarker)}</strong></td>
                    <td>${t.previous} ${escapeHtml(t.unit || "")}</td>
                    <td>${t.latest} ${escapeHtml(t.unit || "")}</td>
                    <td style="color:${diffColor}; font-weight:bold; display:flex; align-items:center; gap:4px;">${diffIcon}${diffPrefix}${t.change} ${escapeHtml(t.unit || "")}</td>
                </tr>`;
                });
        ...
        function renderFileQueue(files) {
            ...
            <div class="file-queue-item" id="queue-item-${escapeHtml(f.name)}">
                <div class="file-queue-left">
                    <span class="file-queue-icon">${svgIcon("file")}</span>
                    <div>
                ...
                <div class="file-queue-right">
                    <span class="badge token-badge" title="Cryptic Content Token: ${escapeHtml(f.token || 'N/A')}">
                        ${svgIcon("lock")} ${f.token ? escapeHtml(f.token.slice(0, 15)) + '...' : 'Processing'}
                    </span>
                    <button class="btn-remove-queue" data-filename="${escapeHtml(f.name)}" title="Remove this report from queue">${svgIcon("x")}</button>
                </div>
            </div>
        `
                )
            ...
            try {
                ...
                if (inspector && inspectorContent && data.retrieval_diagnostics) {
                    ...
                    ${data.extracted_claims ? `
                        ...
                        <pre style="background:var(--bg-1); padding:8px; border-radius:4px; font-size:11px;">${escapeHtml(JSON.stringify(data.extracted_claims, null, 2))}</pre>
                    ` : ''}
                `;
                }
                ...
                appendMessage({
                    role: "assistant",
                    text: `**Error:** ${err.message || "Could not reach the G1Saathi server."}`,
                });
            } finally {
        ...
        function appendMessage({ role, text, sources, safetyAlert, agentTrace, evidenceMetrics, taskPlan, toolCalls, nextSteps, informationGap }) {
            ...
            avatar.innerHTML = svgIcon(role === "user" ? "user" : "logo");
            ...
            if (alertText) {
                ...
                alertBox.innerHTML = `
                <div class="safety-alert-icon">${svgIcon("alert")}</div>
            ...
            if (sources && sources.length > 0) {
                ...
                if (hasExternal) {
                    badgeDiv.innerHTML = `<span style="color: var(--info); display:inline-flex; align-items:center; gap:4px;">${svgIcon("globe")} Live Research Used</span>`;
                } else {
                    badgeDiv.innerHTML = `<span style="color: var(--success); display:inline-flex; align-items:center; gap:4px;">${svgIcon("book")} Knowledge Base</span>`;
                }
                ...
                toggleBtn.innerHTML = `<span>${svgIcon("bookmark")} Cited Sources (${sources.length})</span> ${chevronToggle()}`;
                ...
                if (localSources.length > 0) {
                    ...
                    localHeader.innerHTML = `<strong>${svgIcon("book")} Local Knowledge</strong>`;
                ...
                if (extSources.length > 0) {
                    ...
                    extHeader.innerHTML = `<strong style="margin-top: 8px; display: block;">${svgIcon("globe")} External Research</strong>`;
                ...
                toggleBtn.addEventListener("click", () => {
                    ...
                    toggleBtn.querySelector(".chevron").classList.toggle("is-open", isHidden);
                });
            ...
            if (agentTrace && agentTrace.length > 0) {
                ...
                toggleBtn.innerHTML = `<span>${svgIcon("search")} Agent Workflow</span> ${chevronToggle()}`;
                ...
                let traceHtml = agentTrace.map(step => {
                    ...
                    const cleanStep = step.replace(/^[✗✓]\s*/, "").replace(/[✗✓]/g, "").trim();
                    return `<div class="trace-item ${isErr ? 'trace-item-err' : 'trace-item-ok'}" style="font-size: 0.9em; padding: 4px 8px; display:flex; align-items:center; gap:6px;">${svgIcon(isErr ? "x-circle" : "check-circle")} ${escapeHtml(cleanStep)}</div>`;
                }).join("");
            ...
            if (informationGap && informationGap.status === "MISSING_INFORMATION") {
                ...
                gapBox.style.background = "var(--warning-soft)";
                gapBox.style.borderColor = "var(--warning)";
                gapBox.innerHTML = `
                <div class="safety-alert-icon" style="color: var(--warning);">${svgIcon("help-circle")}</div>
                <div class="safety-alert-content" style="color: #6b4a12;">
            ...
            if (nextSteps && nextSteps.length > 0) {
                ...
                stepsToggleBtn.innerHTML = `<span>${svgIcon("lightbulb")} Suggested Next Steps</span> ${chevronToggle()}`;
                ...
                stepsToggleBtn.addEventListener("click", () => {
                    ...
                    stepsToggleBtn.querySelector(".chevron").classList.toggle("is-open", isHidden);
                });
            ...
            if (evidenceMetrics && role === "assistant") {
                ...
                mToggleBtn.innerHTML = `<span>${svgIcon("bar-chart")} Evidence Quality</span> ${chevronToggle()}`;
                ...
                let statusIcon = svgIcon("check-circle");
                let statusText = evidenceMetrics.evidence_status;

                if (evidenceMetrics.evidence_status === "INSUFFICIENT") {
                    statusColor = "var(--danger)";
                    statusIcon = svgIcon("alert");
                    statusText = "INSUFFICIENT<br><small style='color:var(--text-muted);'>The available knowledge does not contain enough supporting information for a reliable answer.</small>";
                } else if (evidenceMetrics.evidence_status === "CONFLICTING") {
                    statusColor = "var(--warning)";
                    statusIcon = svgIcon("alert");
                    statusText = "CONFLICTING<br><small style='color:var(--text-muted);'>Relevant sources contain differing information.</small>";
                ...
                <div style="border-top: 1px solid var(--border-color); padding-top: 8px; margin-top: 8px;">
                    <div style="color: var(--text-muted); font-size: 0.8em; margin-bottom: 4px;">Status</div>
                    <div style="color: ${statusColor}; font-weight: bold; display:flex; align-items:center; gap:6px;">
                        ${statusIcon} ${statusText}
                    </div>
                </div>
                <div style="margin-top: 10px; font-size: 0.75em; color: var(--text-muted); background: var(--bg-1); padding: 6px; border-radius: 4px; display:flex; align-items:flex-start; gap:5px;">
                    ${svgIcon("info")} <span>Evidence coverage indicates how well the response is supported by retrieved sources. It is not a medical confidence score, diagnosis probability, or assessment of patient health.</span>
                </div>
            `;
                ...
                mToggleBtn.addEventListener("click", () => {
                    ...
                    mToggleBtn.querySelector(".chevron").classList.toggle("is-open", isHidden);
                });
        ...
        function showTypingIndicator() {
            ...
            row.innerHTML = `
            <div class="message-avatar">${svgIcon("logo")}</div>
            ...
            try {
                ...
                if (!res.ok) {
                    if (elements.progressStaticIcon) {
                        ...
                        elements.progressStaticIcon.innerHTML = svgIcon("x-circle");
                    }
                ...
                if (elements.progressStaticIcon) {
                    elements.progressStaticIcon.style.display = "inline-block";
                    elements.progressStaticIcon.innerHTML = svgIcon("check-circle");
                }
                ...
                if (data.rejected && data.rejected.length > 0) {
                    data.rejected.forEach((r) => {
                        showToast(`${r.name}: ${r.reason}`, "error");
                    });
                }
        ...
        async function runIndexing() {
            ...
            try {
                ...
                const warnText = (data.scanned_files && data.scanned_files.length > 0)
                    ? ` (Scanned PDF warning: ${data.scanned_files.join(', ')})`
                    : "";
        ...
        function showToast(message, type = "info") {
            ...
            toast.innerHTML = `<span class="toast-icon">${svgIcon(iconName)}</span> <span>${escapeHtml(message)}</span>`;
        ...
        function chevronToggle(cls) {
            return `<span class="chevron${cls ? " " + cls : ""}">${svgIcon("chevron-down")}</span>`;
        }
